...
df_truth <- data.frame(
  ...
  component = factor(
    rep(c("Sparse (beta)", "Dense (delta)", "Total (theta = beta + delta)"), each = p),
    levels = c("Sparse (beta)", "Dense (delta)", "Total (theta = beta + delta)")
  )
)
...
df_lava_dense <- data.frame(
  ...
  fill_grp = "LAVA dense (delta-hat)"
)
df_lava_sparse <- data.frame(
  method   = factor("LAVA (BIC)", levels = method_levels),
  index    = 1:p,
  value    = beta_hat,
  fill_grp = "LAVA sparse (beta-hat)"
)
...
fill_palette <- c(
  ...
  "Lasso"                  = "#56B4E9",
  "Elastic-net"            = "#E69F00",
  "LAVA dense (delta-hat)" = "#CC79A7",
  "LAVA sparse (beta-hat)" = "#009E73"
)
...
ggplot(mapping = aes(x = index, y = value, fill = fill_grp)) +
  ...
  scale_fill_manual(
    values = fill_palette,
    breaks = c("LAVA sparse (beta-hat)", "LAVA dense (delta-hat)"),

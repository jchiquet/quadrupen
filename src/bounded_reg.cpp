...
Rcpp::List bounded_reg_cpp(
  ...
  const double lambda_2     = tuningParam["lambda_l2"]   ; // scalar for the amount L2 penalty
  ...
  for (uword m=0; m<n_lambda; m++) {
    ...
    // degress of freedom
    df[m] = get_df_breg(lambda_2, xtx, S, I);

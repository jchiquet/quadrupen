...
test_that("weighted_ quad2theo", {
  ...
  expect_equal(as.matrix(lasso.quad$coefficients), theo.path, check.attributes = FALSE)
  ...
  expect_equal(as.matrix(t(lasso.glmn$beta)), as.matrix(lasso.quad$coefficients), tolerance = 1e-6, check.attributes = FALSE)
  
  ## Check the intercept term also
  expect_equal(lasso.glmn$a0, lasso.quad$interceptTerm, tolerance = 1e-6, check.attributes = FALSE)

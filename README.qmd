...
```{r ridge_nostruct}
plot(ridge(xx,y, intercept=FALSE), labels=labels)
```

#### Lasso  ($\ell_1$ penalty)

```{r lasso_nostruct}
plot(lasso(xx,y, intercept=FALSE), labels=labels)
```

#### Elastic-net  ($\ell_1+\ell_2$ penalty)

```{r enet_nostruct}
plot(elastic.net(xx,y, lambda2=1, intercept=FALSE), labels=labels)
```

#### Bounded regression ($\ell_\infty$ penalty)

```{r breg_nostruct}
plot(bounded.reg(xx,y, lambda2=0, intercept=FALSE), labels=labels)
```

#### Ridge + Bounded regression ($\ell_\infty+\ell_2$ penalty)

```{r bregridge_nostruct}
plot(bounded.reg(xx,y, lambda2=5, intercept=FALSE), labels=labels)
```
...
```{r ridge_struct}
plot(ridge(xx,y, lambda = 10^seq(4,-1,len=100), struct = L, intercept=FALSE), labels=labels)
```
